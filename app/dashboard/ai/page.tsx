...
export default function AgroAIPage() {
  ...
  const handleAskAboutScan = () => {
    setActiveTab("chat");
    setChatMessages([
      { from: "ai", text: "I analyzed your photo and detected Early Blight with 94% confidence. I recommend applying Mancozeb fungicide and improving airflow. What else would you like to know about managing this?" }
    ]);
  };
                  ...
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-3 mt-5">
                        <button 
                          onClick={handleAskAboutScan}
                          className="w-full sm:w-auto bg-brand-green text-white font-extrabold px-5 py-2.5 rounded-xl hover:bg-brand-green-hover transition-colors flex items-center justify-center gap-2"
                        >
                        ...
                        <button 
                          ...
                          className="w-full sm:w-auto text-sm font-extrabold text-neutral-500 hover:text-neutral-700 transition-colors flex items-center justify-center gap-1.5 px-4 py-2"
                        >
          ...
          <div className="max-w-3xl mx-auto h-[500px] flex flex-col bg-neutral-50 rounded-2xl overflow-hidden border border-neutral-100">
            <div className="flex-1 overflow-y-auto p-6 space-y-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {chatMessages.map((m, i) => (
                <div key={i} className={`flex w-full ${m.from === "ai" ? "justify-start" : "justify-end"}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl text-sm font-medium leading-relaxed shadow-sm ${m.from === "ai" ? "bg-white border border-neutral-100 text-neutral-800 rounded-tl-sm" : "bg-brand-green text-white rounded-tr-sm"}`}>
                    {m.from === "ai" && (
